...
import org.opensearch.integrations.model.RestTag.NAME_FIELD
import org.opensearch.integrations.model.RestTag.QUERY_FIELD
 ...
 */
internal class IntegrationQueryHelper(private val types: EnumSet<IntegrationObjectType>) {
    companion object {
        ...
        // keyword and text fields are under Integration object and should be prepended with type
        ...
        private val INTEGRATION_OBJECT_FIELDS = KEYWORD_FIELDS.union(TEXT_FIELDS)
        private val ALL_FIELDS = METADATA_FIELDS.union(INTEGRATION_OBJECT_FIELDS)

        val FILTER_PARAMS = ALL_FIELDS.union(setOf(QUERY_FIELD))
    }

    private val prefixes = if (types.size == 0) {
        IntegrationObjectType.getAll()
    } else {
        types
    }
    ...
    private fun getQueryAllBuilder(queryValue: String): QueryBuilder {
        ...
        INTEGRATION_OBJECT_FIELDS.forEach {
