 ...
 */
internal class ObservabilityQueryHelper(
    private val types: EnumSet<ObservabilityObjectType>,
) {
    companion object {
    ...
    fun addQueryFilters(
        ...
        filterParams.forEach {
            when {
                QUERY_FIELD == it.key -> query.filter(getQueryAllBuilder(it.value))
    ...
    private fun getRangeQueryBuilder(
        ...
        return when (range.size) {
            1 -> {
                QueryBuilders.termQuery(queryKey, queryValue)
            }
    ...
    private fun addTermsQueryBuilder(
        query: BoolQueryBuilder,
        queryKey: String,
        queryValue: String,
    ) {
    ...
    private fun addMatchQueryBuilder(
        query: BoolQueryBuilder,
        queryKey: String,
        queryValue: String,
    ) {
