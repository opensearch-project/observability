...
/*
 * "MainButtons" component is used to populate notebook related buttons on right top of the plugin page.
 *
...
export const MainButtons = (props: MainButtonsProps) => {
  ...
  const onCreate = async (input: string) => {
    createNotebook(input);
    closeModal();
  };

  const onRename = async (input: string) => {
    renameNotebook(input, openNoteId);
    closeModal();
  };
  ...
  const onImport = async (file: FileList) => {
    ...
    fr.onload = function (e: ProgressEvent<FileReader>) {
      try {
        fileObj = JSON.parse(e.target.result.toString());
        importNotebook(fileObj);
      } catch {
